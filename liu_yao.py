...
def get_he_type(month_or_day_branch: str, yao_branch: str, is_month: bool = True) -> Optional[str]:
    ...
    - 克合：巳火->申金、卯木->戌土、丑土->子水
    - 平合：未土->午火、酉金->辰土、寅木->亥水、申金->巳火、戌土->卯木、子水->丑土
...
class YaoDetails:
    ...
    ri_he: Optional[str] = None  # 日合类型（"生合"、"克合"、"平合"）
    ...
    changed_ri_he: Optional[str] = None  # 變卦日合类型（"生合"、"克合"、"平合"）
...
HEXAGRAM_MAP: Dict[str, HexagramInfo] = {
    ...
    "010000": HexagramInfo("地水師", "興師動眾", "水", 3, 6, True, "坎", "坎", "坤", "歸魂"),
    ...
    "010011": HexagramInfo("風水渙", "渙散分離", "火", 5, 2, False, "離", "坎", "巽", ""),
    "010111": HexagramInfo("天水訟", "爭訟糾紛", "火", 4, 1, False, "離", "坎", "乾", "游魂"),
    "101111": HexagramInfo("天火同人", "同心協力", "火", 3, 6, False, "離", "離", "乾", "歸魂"),
    ...
    "110100": HexagramInfo("雷澤歸妹", "婚嫁之道", "金", 3, 6, False, "兌", "兌", "震", "歸魂"),
}
...
def format_liu_yao_display_pc(yao_list: List[YaoDetails], show_shen_sha: bool = True, for_gradio: bool = False, show_tian_gan: bool = False) -> str:
    ...
    for idx in range(5, -1, -1):
        ...
        # 5. 日合系列（日生合/日克合/日平合/日合）- 優先於日生、日克
        # 6. 臨日 - 獨立標記
        # 7. 日扶 - 需排除日沖（日沖優先）
        # 8. 日生 - 需排除日生合（日生合優先）
        ...
        if yao.changed_pillar is not None:
            ...
            if yao.is_changing:
                ...
                if yao.changed_ri_he:
                    ...
                    elif yao.changed_ri_he == "平合":
                        changed_marker_parts.append("日平合")
                    else:
