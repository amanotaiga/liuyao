...
def format_liu_yao_display(yao_list: List[YaoDetails], show_shen_sha: bool = True, for_gradio: bool = False) -> str:
    ...
    # Pre-calculate: check if all relatives are present (used in multiple places)
    main_relatives = set()
    for yao in yao_list:
        if yao.main_relative and yao.main_relative not in ["錯誤", "未知", ""]:
            main_relatives.add(yao.main_relative)
    ...
    # Cache for display_width calculations
    display_width_cache = {}
    def cached_display_width(s: str) -> int:
        if s not in display_width_cache:
            display_width_cache[s] = display_width(s)
        return display_width_cache[s]
    
    # Combined first pass: collect base strings, marker widths, shen_sha_texts, and hidden_strs
    # This reduces multiple passes through yao_list
    base_strings = []
    marker_widths = []
    marker_parts_list = []  # Cache marker_parts to avoid recalculation
    ...
    for i in range(5, -1, -1):
        ...
        # Calculate marker_parts once and cache it (used again in second pass)
        marker_parts = []
        if main_shi_ying:
        ...
        # Cache marker_parts for reuse in second pass
        marker_parts_list.append(marker_parts)
        ...
        if marker_parts:
            ...
            marker_widths.append(cached_display_width(markers_str))
        else:
            marker_widths.append(0)
        
        # Also calculate shen_sha_texts and hidden_strs in the same pass
        if show_shen_sha:
            markers = []
            # 不包含旺衰信息（臨月、月扶、月生、月克等）、月破、日沖、月合、日合、臨日、日扶、日生、日克、旬空和回頭生/克
            # 过滤掉化進神/化退神和回頭生/回頭克
            ...
            shen_sha_texts.append("")
        
        # Calculate hidden_strs in the same pass
        hidden_str = ""
        ...
        if yao.changed_pillar is not None:
            ...
            # Calculate marker width: " {changed_shi_ying} {hua_jin_tui} {hui_tou} {day/month relationships}"
            ...
            if yao.is_changing:
                ...
                # 添加日生和日克到變卦欄位（日克合優先於日克）
                if yao.changed_ri_sheng:
                    changed_marker_parts.append("日生")
                if yao.changed_ri_ke and yao.changed_ri_he != "克合":
                    changed_marker_parts.append("日克")
            
            if changed_marker_parts:
                changed_marker_str = " " + " ".join(changed_marker_parts)
                changed_marker_width = cached_display_width(changed_marker_str)
            else:
                changed_marker_str = " "
                changed_marker_width = cached_display_width(changed_marker_str)  # Use display width for consistency
    ...
    max_base_bengua_width = max(cached_display_width(s) for s in base_strings) if base_strings else 0
    ...
    max_base_biangua_width = max(cached_display_width(s) for s in base_changed_strings) if base_changed_strings else 0
    ...
    # shen_sha_texts already calculated in first pass
    if show_shen_sha:
        max_shen_sha_width = max(cached_display_width(text) for text in shen_sha_texts) if shen_sha_texts else 0
    ...
    # Reuse cached marker_parts from first pass to avoid recalculation
    for i, base_str in enumerate(base_strings):
        yao = yao_list[5 - i]  # Reverse index to match the original order
        
        # Reuse cached marker_parts instead of recalculating
        marker_parts = marker_parts_list[i]
    ...
    # all_relatives_present and hidden_strs already calculated in first pass
    ...
    # Use cached display_width for better performance
    max_bengua_display_width = max(cached_display_width(s) for s in hexagram_strings) if hexagram_strings else 0
    max_biangua_display_width = max(cached_display_width(s) for s in changed_strings) if changed_strings and any(s for s in changed_strings) else 0
    max_shensha_display_width = max(cached_display_width(s) for s in shen_sha_texts) if show_shen_sha and shen_sha_texts else 0
    col_spirit_display_width = max(cached_display_width(yao.spirit if yao.spirit else "") for yao in yao_list) if yao_list else 4
    
    # hidden_strs already calculated in first pass
    col_fushen_display_width = max(cached_display_width(s) for s in hidden_strs) if hidden_strs else 8
    ...
    # Pre-calculate border segments to avoid repeated function calls
    border_spirit = make_border_segment(col_spirit)
    border_fushen = make_border_segment(col_fushen)
    border_bengua = make_border_segment(max_bengua_chars)
    border_biangua = make_border_segment(max_biangua_chars)
    
    if show_shen_sha:
        border_shensha = make_border_segment(max_shensha_chars)
        # Use join() for better performance
        header_top = "".join(["╔", border_spirit, border_fushen, border_bengua, border_biangua, border_shensha, "╗"])
        ...
        header_sep = "".join(["╠", border_spirit, border_fushen, border_bengua, border_biangua, border_shensha, "╣"])
        footer = "".join(["╚", border_spirit, border_fushen, border_bengua, border_biangua, border_shensha, "╝"])
    else:
        # Use join() for better performance
        header_top = "".join(["╔", border_spirit, border_fushen, border_bengua, "╦", border_biangua, "╗"])
        ...
        header_sep = "".join(["╠", border_spirit, border_fushen, border_bengua, border_biangua, "╣"])
        footer = "".join(["╚", border_spirit, border_fushen, border_bengua, border_biangua, "╝"])
    ...
    for idx, i in enumerate(range(5, -1, -1)):
        ...
        # 如果本卦中六親都已齐全，则不显示伏神
        # hidden_str already calculated in first pass
        hidden_str = hidden_strs[idx]
