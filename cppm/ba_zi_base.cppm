...
struct Pillar {
     ...
     * @throws std::invalid_argument 如果字符串無法映射到枚举
     * 
     * @example
     * Pillar p1("甲", "子");  // 创建甲子
     * Pillar p2("乙", "丑");  // 创建乙丑
...
private:
    ...
    static TianGan string_to_tian_gan(std::string_view str) {
        ...
        throw std::invalid_argument(std::string("無效的天干: ") + std::string(str));
    }
    ...
    static DiZhi string_to_di_zhi(std::string_view str) {
        ...
        throw std::invalid_argument(std::string("無效的地支: ") + std::string(str));
    }
};
