...
struct YaoDetails
{
    ...
    // 变爻/變卦信息 (變卦)
    bool isChanging = false; // 是否为動爻
    Pillar changedPillar; // 变出之爻的干支 (按變卦独立纳甲) (optional)
    ...
    std::string shiYingMark; // 世/應标记 ("世", "應", " ")
    char mainYaoType; // 本卦爻阴阳 ('0'或'1')
    std::string changeMark = " "; // 動爻标记 ('X' 阴变, 'O' 阳变, " ")
...
const std::unordered_map<std::string, HexagramInfo> hexagramMap = {
    // 乾宫（金）系列
    {"111111", {"乾为天", "刚健中正", "金", 6, 3, true, "乾", "乾", "乾", "本宫(六沖)"}},
    {"011111", {"天风姤", "阴阳相遇", "金", 1, 4, true, "乾", "巽", "乾", ""}},
    {"001111", {"天山遁", "退避保全", "金", 2, 5, true, "乾", "艮", "乾", ""}},
    {"000111", {"天地否", "闭塞不通", "金", 3, 6, true, "乾", "坤", "乾", "六合"}},
    ...
    // 坎宫（水）系列
    {"010010", {"坎为水", "险陷重重", "水", 6, 3, true, "坎", "坎", "坎", "本宫(六沖)"}},
    {"110010", {"水泽节", "节制有度", "水", 1, 4, true, "坎", "兑", "坎", "六合"}},
    ...
    // 艮宫（土）系列
    {"001001", {"艮为山", "静止不动", "土", 6, 3, true, "艮", "艮", "艮", "本宫(六沖)"}},
    {"101001", {"山火贲", "文饰美化", "土", 1, 4, true, "艮", "离", "艮", "六合"}},
    ...
    // 震宫（木）系列
    {"100100", {"震为雷", "震动奋发", "木", 6, 3, true, "震", "震", "震", "本宫(六沖)"}},
    {"000100", {"雷地豫", "安乐警惕", "木", 1, 4, true, "震", "坤", "震", "六合"}},
    ...
    // 坤宫（土）系列
    {"000000", {"坤为地", "厚德载物", "土", 6, 3, false, "坤", "坤", "坤", "本宫(六沖)"}},
    {"100000", {"地雷复", "阳气复归", "土", 1, 4, false, "坤", "震", "坤", "六合"}},
    {"110000", {"地泽临", "督导视察", "土", 2, 5, false, "坤", "兑", "坤", ""}},
    {"111000", {"地天泰", "天地交泰", "土", 3, 6, false, "坤", "乾", "坤", "六合"}},
    {"111100", {"雷天大壮", "强盛壮大", "土", 4, 1, false, "坤", "乾", "震", "六沖"}},
    {"111110", {"泽天夬", "果断决策", "土", 5, 2, false, "坤", "乾", "兑", ""}},
    {"111010", {"水天需", "耐心等待", "土", 4, 1, false, "坤", "乾", "坎", "游魂"}},
    {"000010", {"水地比", "亲和依附", "土", 3, 6, false, "坤", "坤", "坎", "归魂"}},

    // 兑宫（金）系列
    {"110110", {"兑为泽", "喜悦沟通", "金", 6, 3, false, "兑", "兑", "兑", "本宫(六沖)"}},
    {"010110", {"泽水困", "困境求生", "金", 1, 4, false, "兑", "坎", "兑", "六合"}},
    {"000110", {"泽地萃", "人才荟萃", "金", 2, 5, false, "兑", "坤", "兑", ""}},
    {"001110", {"泽山咸", "感應相知", "金", 3, 6, false, "兑", "艮", "兑", ""}},
                                                          ...
                                                          const std::vector<int> &changingLineIndices)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        for (int idx : changingLineIndices)
        {
            if (idx == i + 1)
            {
                yaoDetailsList[i].isChanging = true;  // 修复：應该是 [i] 而不是 [i+1]
                break;
            }
        }
        ...
        {
            yaoDetailsList[i].shiYingMark = "應";
        }
        else
        {
...
)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        else
        {
            ...
            yaoList[i].changedRelative = "錯誤";
            std::print(std::cerr, "警告: 無法找到变爻地支 '{}' 的五行属性。\n", hiddenBranch);
        }
    }
}
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (branchFiveElements.contains(branch)) {
            ...
            if (isMainHexagram) {
                ...
                yaoList[i].mainRelative = "錯誤";
            } else {
                yaoList[i].changedElement = "未知";
                yaoList[i].changedRelative = "錯誤";
            }
            std::print(std::cerr, "警告: 無法找到地支 '{}' 的五行属性。\n", branch);
        }
    }
}
                                                                           ...
                                                                           const std::vector<int> &changingLineIndices)
{
    ...
    if (!changingLineIndices.empty()) {
        ...
        for (int idx : changingLineIndices) {
            if (idx >= 1 && idx <= 6) {  // 修复：idx 是 1-6
                ...
                std::print(std::cerr, "警告: 動爻位置超出范围: {}\n", idx);
            }
        }
    ...
    if (!dayStem.empty() && dayStemToSpiritStart.contains(dayStem)) {
        ...
        std::print(std::cerr, "警告：無法计算六神（日干: {}）\n", dayStem);
        for (int i = 0; i < 6; ++i) {
    ...
    std::println("| {:<{}} | {:<{}} | {:<{}} | {:<{}} |", 
                 ...
                 "本卦 (飞神)", bengua_width, "變卦", biangua_width);
    std::println("|{:-<{}}+{:-<{}}+{:-<{}}+{:-<{}}|", 
...
inline nlohmann::json aiSetSixYaoDivination(
    ...
    if (original_json.contains("bian_gua_name")) {
        ai_json["變卦名称"] = original_json["bian_gua_name"];
    }
    ...
    for (std::size_t i = 0z; i < yao_list.size(); ++i) {
        ...
        // 動爻信息（完整翻译）
        yao_cn["是否動爻"] = yao.isChanging;
        //yao_cn["動爻标记"] = yao.changeMark;
        ...
        bian_gua_cn["爻性"] = (yao.mainYaoType == '0' || yao.mainYaoType == 48 ? "阳爻" : "阴爻");  // 變卦爻性相反
        yao_cn["變卦"] = bian_gua_cn;
